...
void writeValueOnLCD(int n, int length);
...
void main(void)
{
	...
	while(1)												// endless 
	{
		...
		LCD_StringWrite("Weight: ", 9);
		LCD_StringWrite(voltage, 4);
		LCD_StringWrite(" mg" ,3);
		LCD_CmdWrite(0x80);
...
void timer0_ISR (void) interrupt 1
{
	...
	adcValue = adcValue + adcVal;
	//Write averaging of 10 samples code here
	if(count%10 == 0)
	{
		avg = (float)adcValue*0.384;
		//avg /= 0.74;
		avgVal = avg;
		adcValue = 0;
		count = 0;
